...
router.post('/api/judge/compile', async (req, res) => {
	// TODO: For now this is only for code written in c++, in the future rewrite this to be more general.	
...
router.post('/api/judge/run', async (req, res) => {
	...
	if (langs[req.body.solution.language].compile) {
		try {
	...
	try {
		const { stdout } = await Run(SOL_DIR,
																 parseInt(req.body.timelimit) / 1000, 
																 parseInt(req.body.memorylimit) * 1024 * 1024, 
																 INPUT_PATH, 
																 langs[req.body.solution.language].run('solution'));
